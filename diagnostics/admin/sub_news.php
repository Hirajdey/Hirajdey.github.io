...
if(isset($_REQUEST['btnadd']) && $_REQUEST['btnadd']){
    ...
    exit;*/
    $insert = $conn->query("INSERT INTO media_subnews (title_news_id, image, pdf, url, created_date) VALUES ('".$_POST['title']."', '".$_POST['image']."', '".$_POST['pdf']."', '".$_POST['url']."', NOW())");
    ...
    header("Location:?act=sub_news&sub=list");
}
elseif(isset($_REQUEST['btnedit']) && $_REQUEST['btnedit']) {
    ...
    $conn->query("UPDATE media_subnews SET title_news_id = '".$_POST['title']."', image = '".$_POST['image']."', pdf = '".$_POST['pdf']."', url = '".$_POST['url']."' WHERE id =  '".$_REQUEST['id']."'");
    header("Location:?act=sub_news&sub=list");
    ...
    $delete = $conn->query("DELETE FROM media_subnews WHERE id = '".$_REQUEST['id']."' LIMIT 1");
    header("Location:?act=sub_news&sub=list");
}

if(isset($_REQUEST['id']) && $_REQUEST['sub'] == 'edit') {
    $edit = $conn->query("SELECT mn.id AS id, ms.image AS image, ms.pdf AS pdf, ms.url AS url FROM media_subnews AS ms JOIN media_news AS mn ON mn.id = ms.title_news_id WHERE ms.id = '".$_REQUEST['id']."'");
    ...
    //echo "DELETE FROM disease_test WHERE id = '".$_REQUEST['id']."' LIMIT 1"; exit;
    $delete = $conn->query("DELETE FROM media_subnews_download_users WHERE id = '".$_REQUEST['id']."' LIMIT 1");
    header("Location:?act=sub_news&sub=list");
}*/
?>
...
<div class="container-fluid">
    ...
    <h2>Sub News</h2><div class="row">
        ...
        <?php if(isset($_REQUEST['sub']) && $_REQUEST['sub'] != "add" && $_REQUEST['sub'] != "edit") {?><div class="text-right"><a href="?act=sub_news&sub=add"><h3>Add</h3></a></div> <br><?php } ?>
    </div>
    ...
    <div class="table-responsive">
        <table class="table table-striped">
            ...
            <tbody>
            <?php $leads = $conn->query("SELECT ms.id AS id, ms.image AS image, ms.pdf AS pdf, ms.url AS url, mn.title AS title, YEAR (mn.news_date) AS year FROM media_subnews AS ms JOIN media_news AS mn ON mn.id = ms.title_news_id ORDER BY ms.id DESC");
            $i = 1;
            while($row = $leads->fetch_assoc()) {?>
            <tr>
                <td><?php echo $i;?>.</td>
                <td><?php echo $row['title'];?></td>
                <td><img src="images/press/<?php echo $row['year'];?>/<?php echo $row['image'];?>" width="250px" height="50"></td>
                <td><a href="pdf/<?php echo $row['year'];?>/<?php echo $row['pdf'];?>" target="_blank"><img src="images/press/pdf-link-icon.jpg"></a></td>
                <td><a href="<?php echo $row['url'];?>" target="_blank"><img src="images/press/source-link-icon.jpg"></a></td>
                <td><a href="?act=sub_news&sub=edit&id=<?php echo $row['id'];?>">Edit</a> / <a href="?act=sub_news&sub=delete&id=<?php echo $row['id'];?>">Delete</a></td>
            </tr>
            <?php $i++; } ?>
            </tbody>
        </table>
    ...
    <?php } elseif((isset($_REQUEST['sub']) && $_REQUEST['sub'] == "add") || (isset($_REQUEST['sub']) && $_REQUEST['sub'] == "edit")){
        $code = $conn->query("SELECT * FROM media_news");
        ?>
        <br>
        <form class="form-horizontal" method="post">
            ...
            <div class="form-group">
                <img src="images/press/2016/<?php echo $_POST['image'];?>" width="250px" height="50">
