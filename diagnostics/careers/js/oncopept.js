...
oncopeptApp.controller("oncopeptCtrl", function ($scope, $http, $location, $modal, $rootScope, $uibModalStack, calculateService) {
    ...
    // email popup open
    $scope.open = function (file, path) { //alert("Site path: " + calculateService.getCurrentUrl());
        var modalInstance = $modal.open({
            controller: 'emailCtrl',
            templateUrl: calculateService.getCurrentUrl() + '/oncopeptresources/emailForm.html',
            //templateUrl: 'http://medgenome.com/oncopeptresources/emailForm.html',
            resolve: {
                file: function () {
                    return file;
                },
...
angular.module('oncopeptApp').controller('emailCtrl', function ($scope, $http, $modal, $modalInstance, $rootScope, calculateService, file, path, $window, act) {
    ...
    $scope.showRegisterForm = function () {
        ...
        var registerModalInstance = $modal.open({
            ...
            templateUrl: calculateService.getCurrentUrl() + '/oncopeptresources/registerForm.html'
        });
    }
    // end
    ...
    $scope.showForgotPasswordForm = function () {
       $scope.close();
         var forgotPasswordModalInstance = $modal.open({
            controller: 'registerCtrl',
            templateUrl: calculateService.getCurrentUrl() + '/oncopeptresources/forgotPasswordForm.html'
        });
    }
    // end
        ...
        if (($scope.loginemail != '') && (typeof($scope.loginemail) != 'undefined') && ($scope.loginpassword != '') && (typeof($scope.loginpassword) != 'undefined')) {
            ...
            $http({
                ...
                if(response == 0){
                    ...
                    $scope.close();
                    $window.location.href = calculateService.getCurrentUrl() + "/oncopeptresources/?act=" + act;
...
angular.module('oncopeptApp').controller('registerCtrl', function ($scope, $http, $modal, $rootScope, $uibModalStack, calculateService){
    ...
    $scope.registerSubmit = function () { alert(calculateService.getCurrentUrl())
        ...
        if (($scope.name != '') && ($scope.email != '') && (typeof($scope.email) != 'undefined') && ($scope.organization != null) && ($scope.password != '') && (typeof($scope.password) != 'undefined')) {
            ...
            var method = 'POST';
            var url = calculateService.getCurrentUrl() + '/oncopeptdownload.php?name=' + $scope.name + '&email=' + $scope.email + '&organization=' + $scope.organization + '&designation=' + $scope.designation + '&password=' + $scope.password;
