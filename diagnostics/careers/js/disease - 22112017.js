angular
    ...
    .controller("diseaseCtrl", function ($scope, $http, $sce, $modal, $rootScope, $uibModalStack, calculateService) { //alert("Disease CTRL")
        ...
        var method = 'POST';
        var url = calculateService.getCurrentUrl() + '/diseasedata.php?category=all';
        ...
        // declare the filter for global
        $scope.filtersymbol = "glyphicon glyphicon-plus-sign";

        // Category display
        $scope.custom = true;
        $scope.toggleCustom = function() {
            if($scope.filtersymbol == "glyphicon glyphicon-plus-sign"){
                $scope.filtersymbol = "glyphicon glyphicon-minus-sign";
            } else {
                $scope.filtersymbol = "glyphicon glyphicon-plus-sign";
            }
            ...
            if (typeof($scope.loginemail) == 'undefined' || ($scope.loginemail == '')){
                ...
                $http({
                    ...
                    if(response == 0){
                        ...
                        var modalInstance = $modal.open({
                            controller: 'emailCtrl',
                            templateUrl: calculateService.getCurrentUrl() + '/disease/geneList.html',
        ...
        $http({
            method: 'POST',
            url: calculateService.getCurrentUrl() + '/diseasedata.php?disease=All',
            //data: FormData,
            headers: {'Content-Type': 'application/x-www-form-urlencoded'}
        ...
        $scope.getDiseaseData = function(url, val, category){ //alert(category.selected);
            ...
            if( storedNames != null && (category.selected == true)) { //alert("NOT NULL")
                ...
                // add category id with previous id in array while check the checkbox
                    ...
                    //alert("Previous Cat: "+previous_category+ "New Category: "+additional_category);
            ...
            // un check process
            if(category.selected == true){ //If it is checked
                //alert('Checked true');
            } else {
            ...
            var method = 'POST';
            var url = calculateService.getCurrentUrl() + '/diseasedata.php?disease='+ total_categories;
