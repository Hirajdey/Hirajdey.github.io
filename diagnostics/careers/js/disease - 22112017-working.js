angular
    ...
    .controller("diseaseCtrl", function ($scope, $http, $sce, $modal, $rootScope, $uibModalStack, calculateService) { //alert("Disease CTRL")
        ...
        $scope.getDiseaseData = function(url, val, category, filter){ //alert(location.selected);

            ////////////// start disease filter section
            if(filter == 'diseasefilter') {
                ...
                /*if(val != 0 && (category.selected == true)){
            ...
            var method = 'POST';
            var url = calculateService.getCurrentUrl() + '/diseasedata.php?disease='+ total_categories + '&location='+total_location;
