...
if(isset($_REQUEST['month']) || isset($_REQUEST['year'])) {
    ...
    $media_news = $conn->query("SELECT * FROM media_news WHERE ". $month_query . $year_query. " AND status = 1 AND description <> '' ORDER BY news_date DESC limit 0,12");
    ...
    if(($media_news->num_rows) > 0) {
        ...
        while($row = $media_news->fetch_assoc()) {
            $data .= ' {"id":"' . $row['id'] . '", "title":"' . $row['title'] . '", "description":"' . stripslashes($row['description']) . '", "year":"' . strtolower(date('Y', strtotime($row['news_date']))) . '", "currentNews":"' . strtolower(date('MY', strtotime($row['news_date']))) . '", "date":"' . date('jS M', strtotime($row['news_date'])) . '"';
            //echo $row['title']."<br>";
            if ($row['id'] > 0) {
