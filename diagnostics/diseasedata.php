...
if(isset($_REQUEST['disease']) || isset($_REQUEST['location'])) { //echo "2"; exit;
    ...
    if(isset($_REQUEST['disease']) && ($_REQUEST['disease'] != 0) && isset($_REQUEST['location']) && ($_REQUEST['location'] != 0))
        $disease_name = "WHERE disease_category_id in ( ".$_REQUEST['disease']." ) AND location in ( ".$_REQUEST['location']." ) AND status=1";
    elseif(isset($_REQUEST['disease']) && ($_REQUEST['disease'] != 0))
        $disease_name = "WHERE disease_category_id in ( ".$_REQUEST['disease']." ) AND status=1";
    elseif(isset($_REQUEST['location']) && ($_REQUEST['location'] != 0))
        $disease_name = "WHERE location in ( ".$_REQUEST['location']." ) AND status=1";
    else
        $disease_name = "";
    ...
    //echo "SELECT $fields FROM disease_test as dt LEFT JOIN gene_list as jl ON dt.id = jl.disease_test_id $disease_name WHERE dt.status = 1 ORDER BY dt.code ASC"; exit;
    ...
    //echo "No. of Rows: ".$diseases->num_rows; exit;
    if($diseases->num_rows > 0) {
        ...
        echo 0;
} elseif(isset($_REQUEST['geneid'])){  // email validation for login
    //$email_check = $conn->query("SELECT me.id FROM medgenome_employee as me, gene_list_non_employee as ne WHERE me.email = '".$_REQUEST['loginemail']."' || ne.email = '".$_REQUEST['loginemail']."'");
        $gene_list = $conn->query("SELECT gl.gene_name, dt.code, dt.test_name, dt.method, dt.tat FROM disease_test as dt LEFT JOIN gene_list as gl ON dt.id = gl.disease_test_id WHERE gl.id = '".$_REQUEST['geneid']."' AND dt.status = 1");
    ...
    echo 1;
}  elseif(isset($_REQUEST['category'])){ //echo 1; exit;
    $diseases = $conn->query("SELECT * FROM disease_category WHERE status=1 ORDER BY id ASC");
